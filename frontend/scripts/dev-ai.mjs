...
if (!process.env.GROQ_API_KEY) console.error("GROQ_API_KEY is missing. Real AI calls will be unavailable.");
const env = { ...process.env, PUBLIC_JOURNEY_MODE: "ai" };
const astroBin = path.join(root, "node_modules", "astro", "bin", "astro.mjs"); const api = spawn(process.execPath, [path.join(root, "server/index.mjs")], { cwd: root, env, stdio: "inherit" }); const web = spawn(process.execPath, [astroBin, "dev"], { cwd: root, env, stdio: "inherit" });
