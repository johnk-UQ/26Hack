...
const npm = process.platform === "win32" ? "npm.cmd" : "npm"; const api = spawn(process.execPath, [path.join(root, "server/index.mjs")], { cwd: root, env: process.env, stdio: "inherit" }); const web = spawn(npm, ["run", "dev:web"], { cwd: root, env: process.env, stdio: "inherit" });
